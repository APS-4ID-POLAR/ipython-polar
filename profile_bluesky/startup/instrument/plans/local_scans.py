...
def one_dichro_step(detectors, step, pos_cache, take_reading=trigger_and_read):
    ...
    for pos in pr_pos_list:
        yield from mv(pr_setup.positioner, pr_pos + pos)
...
def lup(*args, monitor_count=None, detectors=[scalerd], lockin=False,
    ...
    @configure_monitor_decorator(detectors, monitor_count)
    @stage_dichro_decorator(lockin, dichro)
    def _inner_lup():
...
def ascan(*args, monitor_count=None, detectors=[scalerd], lockin=False,
          dichro=False, md=None):
